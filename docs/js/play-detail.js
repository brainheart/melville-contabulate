...
(function () {
  ...
  async function openPlayDetailModal(playId) {
    ...
    const totalParagraphs = play.total_lines || 0;
    const chapters = shape.acts.size || play.num_acts || 0;

    modal.titleEl.textContent = play.title || play.abbr || 'Unknown work';
    modal.metaEl.textContent = `${play.genre || 'Unknown genre'} \u00b7 ${totalWords} words \u00b7 ${totalParagraphs} paragraphs \u00b7 ${chapters} chapters`;
